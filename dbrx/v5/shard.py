...
class AttnWqkvOutProj:
    ...
    def call_wqkv(self, x: mx.array) -> mx.array:
        return x @ next(self.layers["wqkv"]["generator"])

    def call_out_proj(self, x: mx.array) -> mx.array:
        return x @ next(self.layers["out_proj"]["generator"])
...
class AttnScore(nn.Module):
    ...
    def __call__(
        self,
        x: mx.array,
        outsourced: AttnWqkvOutProj,
        mask: Optional[mx.array] = None,
        cache: Optional[Tuple[mx.array, mx.array]] = None,
    ) -> mx.array:

        qkv = outsourced.call_wqkv(x)
        ...
        return outsourced.call_out_proj(output), (keys, values)
...
class NormAttnNorm(nn.Module):
    def __init__(self, args: ModelArgs):
        ...
        self.attn_score = AttnScore(args)
    ...
    def __call__(
        ...
        h, cache = self.attn_score(
            self.norm_1(x), attn_outsourced, mask=mask, cache=cache
        )
        x = h + x
...
class MoeShard:
    ...
    def warm(self) -> None:
        ...
        return mx.sum(mx.stack(xs, axis=0), axis=0)
...
class DBRX(nn.Module):
    def __init__(
        ...
        self.attn_outsourced = attn_outsourced
...
class Generator:
    ...
    def load_model_and_warmer(self) -> tuple[DBRX, Warmer]:
        ...
        mx.eval(wqkv, out_proj)
        attn_outsourced = AttnWqkvOutProj(wqkv, out_proj)
