...
class DistributedSparseMoeBlock(nn.Module):
    ...
    def design_shard_jobs(self, inds: list[list[int]]) -> dict:
        ...
        for activated_experts in inds:
            ...
            for url, job in d.items():
                shard_jobs[url].append([job, max_load - len(job)])

        return {url: pickle.dumps(jobs) for url, jobs in shard_jobs.items()}

    async def execute_on_shard(
        self,
        shard: moe_shard_lean_pb2_grpc.MoeShardStub,
        x_bytes: bytes,  # x.shape == (batch_size, self.d_model)
        jobs_bytes: bytes,
    ) -> tuple[mx.array, dict]:
        outputs = await shard.Execute(
            moe_shard_lean_pb2.Inputs(data=x_bytes, jobs=jobs_bytes)
        )
        return bytes_to_mx(outputs.data), pickle.loads(outputs.arr_map)
    ...
    async def __call__(self, x: mx.array) -> mx.array:
        ...
        mx.eval(inds, scores)
        inds = inds.tolist()
        shard_jobs = self.design_shard_jobs(inds)  # already in bytes
        ...
        for bi, st, it in zip(range(x.shape[0]), scores, inds):
            ...
            for e in it:
                url = self.expert_map[e]
                expert_outs, arr_map = exec_tasks[url].result()
                yt.append(expert_outs[arr_map[f"{bi}.{e}"]])
...
class Driver:
    ...
    def get_model_args(self) -> ModelArgs:
        ...
        for url, assigned_experts in model_args.ffn_config["moe_shard_map"].items():
    ...
    async def start(self, prompt: str, max_tokens: int, temp: float) -> None:
        async with AsyncExitStack() as es:
            ...
            for url in model_args.ffn_config["moe_shard_map"]:
