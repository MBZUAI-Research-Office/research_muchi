...
def get_driver_urls(config_path: Path) -> list:
    try:
...
async def start(config_path: str, prompt: str, max_tokens: int) -> None:
    ...
    driver_urls = get_driver_urls(Path(config_path))
    async with AsyncExitStack() as es:
        ...
        for url in driver_urls:
            ...
            driver = local_driver_pb2_grpc.LocalDriverStub(channel)
            drivers.append(driver)

        async with asyncio.TaskGroup() as tg:
            inference_tasks = []
            for driver in drivers:
                inference_tasks.append(tg.create_task(call(driver, prompt, max_tokens)))
