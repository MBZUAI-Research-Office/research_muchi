...
class MoeShard:
    ...
    async def __call__(self, inputs: mx.array, block_num: int) -> None:
        # sample jobs:
        # [[{14}, 1], [{}, 2]]
        # for each job,
        # job[0] indicates activated experts in this shard for inputs[i]
        ...
        print(f"started expert calc", flush=True)
        tic = time.perf_counter()
        ...
        print(
            f"ended expert calc, took {(time.perf_counter() - tic):.3f} sec(s)",
            flush=True,
        )
...
class DecoderLayer(nn.Module):
    ...
    async def __call__(self, x: mx.array, shard: MoeShard) -> mx.array:
        await self.ffn(x, shard, self.block_num, self.buffer, self.sync_complete)
