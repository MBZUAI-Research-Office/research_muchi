...
def main():
    ...
    for e, port in enumerate(ports):
        ...
        Cmd(f"""tmux send-keys -t {e} 'cd ~/research_muchi/dbrx/v1_batch1' Enter \;""")
        Cmd(
            f"""tmux send-keys -t {e} 'python moe_shard_batch1.py --port {port}"""
            + f""" --model-path ~/dbrx-instruct/distributable/batch1"""
