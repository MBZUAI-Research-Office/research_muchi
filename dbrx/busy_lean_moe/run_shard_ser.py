...
def main():
    ...
    Cmd(f"""tmux send-keys -t 0 'cd ~/research_muchi/dbrx/busy_lean_moe' Enter \;""")
    Cmd(
        f"""tmux send-keys -t 0 'python moe_shard_lean.py --port {args.port}"""
        + f""" --model-path ~/dbrx-base/distributable/batch2"""
        + f""" --config-filename moe_shard_lean_config.json' Enter \;""",
    )
