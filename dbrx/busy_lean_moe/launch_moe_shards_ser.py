...
def main():
    ...
    try:
        with open(Path(args.model_path) / "driver_lean_config.json", "r") as f:
            config = json.load(f)
    except FileNotFoundError:
        raise

    moe_shard_addr_split = []
    for url in config["ffn_config"]["moe_shard_map"]:
        pure_url, port = url.split(":")
        moe_shard_addr_split.append((pure_url, port))
